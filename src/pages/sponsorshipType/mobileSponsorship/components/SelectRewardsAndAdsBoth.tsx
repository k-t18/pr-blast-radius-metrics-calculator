...
interface SelectRewardsAndAdsBothProperties {
    categoriesData: CategoryItem[];
}

export default function SelectRewardsAndAdsBoth({ categoriesData }: SelectRewardsAndAdsBothProperties) {
    ...
    const handleToggleCategory = (_id: string, title: string, _isMain: boolean) => {
        handleSelectCategory(_id, title);
    };
        ...
        <>
            <div className="flex flex-col gap-6 px-6">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <DescriptionText
                        text="Select at least one category from each sponsorship type."
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {mainCategories.map(item => (
                            <CategoryCard
                                ...
                                onClick={() => handleToggleCategory(item.id || '', item.title || '', true)}
                            />
                        ))}
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {normalCategories.map(item => (
                            <CategoryCard
                                ...
                                tagIcon={item.tagIcon}
                                isSelected={categories.includes(item.title || '')}
                                onClick={() => handleToggleCategory(item.id || '', item.title || '', false)}
                            />
                        ))}
                    </div>
                )}
